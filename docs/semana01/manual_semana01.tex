...
Trabajaremos con el dataset \texttt{datos\_sensores.csv} (originalmente Iris), que contiene medidas morfológicas.
Las columnas son: \texttt{sepal\_length, sepal\_width, petal\_length, petal\_width, species}.

\begin{enumerate}
    \item \textbf{Exploración Categórica:}
    Obtén la lista única de especies (\texttt{species}, columna 5) y cuenta cuántos registros existen por cada una.
    \begin{lstlisting}[style=bashstyle]
# Pista: usa cut, sort y uniq -c
tail -n +2 datos_sensores.csv | cut -d',' -f5 | ...
    \end{lstlisting}

    \item \textbf{Conteo Total:}
    Verifica si el dataset está completo contando el número total de filas de datos (excluyendo el encabezado).

    \item \textbf{Filtrado Numérico (Nuevo):}
    Encuentra cuántas flores tienen un \texttt{petal\_length} (columna 3) mayor a 1.5 cm.
    \begin{lstlisting}[style=bashstyle]
# Pista: usa awk con una condición condicional
awk -F',' '$3 > 1.5 {print $0}' datos_sensores.csv | wc -l
    \end{lstlisting}

    \item \textbf{Detección de Outliers (Reto):}
    Identifica las 3 flores con el \texttt{sepal\_width} (columna 2) más grande de todo el archivo.
    \begin{lstlisting}[style=bashstyle]
...
# 0) (Recomendado) Ir a la raíz del repositorio para que las rutas funcionen
...
# 3) Ver las últimas 5 líneas (útil para validar que el archivo no está cortado)
...
\begin{lstlisting}[style=bashstyle]
# Contar cuántas veces aparece un dato id_sensor en el archivo, conteo rápido de los 10 sensores más frecuentes
cut -d',' -f1 data/raw/datos_sensores.csv | sort | uniq -c | sort -nr | head
...
Construye un pipeline usando \texttt{cut}, \texttt{sort} y \texttt{head} para analizar las flores más grandes. El objetivo es:

\begin{enumerate}
    \item Extraer solo las columnas de \textbf{especie} (columna 5) y \textbf{longitud del pétalo} (columna 3).
    \item Ordenar los resultados por longitud de pétalo de forma \textbf{descendente} (de mayor a menor).
    \item Mostrar en pantalla las \textbf{5 flores con los pétalos más largos} del dataset.
\end{enumerate}

\textbf{Comando esperado:}
\begin{lstlisting}[style=bashstyle]
# Pista: Recuerda que sort necesita saber qué columna usar (-k) o recibir solo los datos a ordenar
cut -d',' -f5,3 data/raw/datos_sensores.csv | sort -t',' -k2 -nr | head -n 5
\end{lstlisting}
\end{agrobox}

\subsection{Procesamiento con awk}

\texttt{awk} es un mini-lenguaje potente para procesar columnas y calcular estadísticas sin abrir Excel. Vamos a usarlo para analizar las dimensiones de las flores.

\begin{lstlisting}[style=bashstyle]
# Calcular la longitud promedio del sépalo (columna 1)
# NR = Número de Registro (filas totales procesadas)
awk -F',' 'NR>1 {s+=$1} END {print "Longitud promedio sépalo:", s/(NR-1)}' data/raw/datos_sensores.csv

# Encontrar el ancho de pétalo máximo (columna 4)
awk -F',' 'NR>1 {if($4>max) max=$4} END {print "Ancho máximo pétalo:", max}' data/raw/datos_sensores.csv

# Filtrar flores pequeñas: mostrar solo aquellas con largo de pétalo < 1.5 cm (columna 3)
awk -F',' '$3 < 1.5 {print $0}' data/raw/datos_sensores.csv | head
\end{lstlisting}

\begin{agrobox}{Ejercicio con awk (30 min)}
Usando el archivo \texttt{data/raw/datos\_sensores.csv}:
\begin{enumerate}
    \item Usa \texttt{awk} para calcular el \textbf{promedio del ancho de sépalo} (columna 2).
    \item Cuenta cuántas flores son de la especie \textbf{"virginica"} (columna 5).
    \item Filtra todas las flores con \texttt{petal\_length > 5.0} y guarda esas filas en un nuevo archivo llamado \texttt{flores\_gigantes.csv}.
\end{enumerate}
\end{agrobox}
...
echo "echo 'Contando filas en el dataset maestro...'" >> scripts/validar_datos.sh
...
\begin{enumerate}
    \item Crea un archivo nuevo \texttt{scripts/resumen.sh} que imprima las primeras 5 líneas de tus datos usando \texttt{head}.
    \item Ejecuta \texttt{git status} para confirmar que Git detecta el archivo nuevo.
    \item Añádelo al área de preparación con \texttt{git add scripts/resumen.sh}.
    \item Haz un commit con el mensaje: \texttt{"feat: script para previsualizar datos"}.
    \item Modifica el archivo \texttt{README.md} agregando una línea al final y haz un nuevo commit.
    \item Usa \texttt{git log} para comprobar que tus 2 nuevos commits aparecen en la lista.
\end{enumerate}
\end{agrobox}
